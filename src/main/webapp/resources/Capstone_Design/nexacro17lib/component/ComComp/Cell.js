...
if (!nexacro._CellControl) {
	...
	_pCellControl._setComputedStyle = function (ctrl_cs) {
		...
		for (var i = 0; i < properties.length; i++) {
			var func = "set_" + properties[i];
			var val = ctrl_cs.styles[properties[i]];
	...
	_pCellBar._setComputedStyle = function (ctrl_cs) {
		...
		for (var i = 0; i < properties.length; i++) {
			...
			if (progressbaritem) {
				val = ctrl_cs.progressbaritem.styles[properties[i]];
				if (progressbaritem[func]) {
					progressbaritem[func](val);
				}
			}
			if (progressbartext) {
				val = ctrl_cs.progressbartext.styles[properties[i]];
				if (progressbartext[func]) {
					progressbartext[func](val);
				}
			}
			if (progressstartcap) {
				val = ctrl_cs.progressstartcap.styles[properties[i]];
				if (progressstartcap[func]) {
					progressstartcap[func](val);
				}
			}
			if (progressendcap) {
				val = ctrl_cs.progressendcap.styles[properties[i]];
				if (progressendcap[func]) {
					progressendcap[func](val);
				}
			}
		}
	};
	...
	_pCellTextArea._setComputedStyle = function (ctrl_cs) {
		...
		for (var i = 0; i < properties.length; i++) {
			...
			if (hscrollbar) {
				val = ctrl_cs.hscrollbar.styles[properties[i]];
				if (hscrollbar[func]) {
					hscrollbar[func](val);
				}

				val = ctrl_cs.hscrollbar.decbutton.styles[properties[i]];
				if (hscrollbar.decbutton[func]) {
					hscrollbar.decbutton[func](val);
				}

				val = ctrl_cs.hscrollbar.incbutton.styles[properties[i]];
				if (hscrollbar.incbutton[func]) {
					hscrollbar.incbutton[func](val);
				}

				val = ctrl_cs.hscrollbar.trackbar.styles[properties[i]];
				if (hscrollbar.trackbar[func]) {
					hscrollbar.trackbar[func](val);
				}
			}
			if (vscrollbar) {
				val = ctrl_cs.vscrollbar.styles[properties[i]];
				if (vscrollbar[func]) {
					vscrollbar[func](val);
				}

				val = ctrl_cs.vscrollbar.decbutton.styles[properties[i]];
				if (hscrollbar.decbutton[func]) {
					hscrollbar.decbutton[func](val);
				}

				val = ctrl_cs.vscrollbar.incbutton.styles[properties[i]];
				if (hscrollbar.incbutton[func]) {
					hscrollbar.incbutton[func](val);
				}

				val = ctrl_cs.vscrollbar.trackbar.styles[properties[i]];
				if (hscrollbar.trackbar[func]) {
					hscrollbar.trackbar[func](val);
				}
			}
	...
	_pCellCalendar._setComputedStyle = function (ctrl_cs) {
		...
		for (var i = 0; i < properties.length; i++) {
			...
			if (calendaredit) {
				val = ctrl_cs.calendaredit.styles[properties[i]];
				if (calendaredit[func]) {
					calendaredit[func](val);
				}
			}
			if (dropbutton) {
				val = ctrl_cs.dropbutton.styles[properties[i]];
				if (dropbutton[func]) {
					dropbutton[func](val);
				}
			}
			if (calendarspinupbutton) {
				val = ctrl_cs.calendarspinupbutton.styles[properties[i]];
				if (calendarspinupbutton[func]) {
					calendarspinupbutton[func](val);
				}
			}
			if (calendarspindownbutton) {
				val = ctrl_cs.calendarspindownbutton.styles[properties[i]];
				if (calendarspindownbutton[func]) {
					calendarspindownbutton[func](val);
				}
			}
		}
	};
	...
	_pCellCombo._setComputedStyle = function (ctrl_cs) {
		...
		for (var i = 0; i < properties.length; i++) {
			...
			if (comboedit) {
				val = ctrl_cs.comboedit.styles[properties[i]];
				if (comboedit[func]) {
					comboedit[func](val);
				}
			}
