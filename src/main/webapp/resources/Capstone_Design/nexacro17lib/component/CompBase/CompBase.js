...
if (!nexacro.Component) {
	...
	_pComponent.on_apply_cssselector = function () {
		...
		if (control_elem) {
			...
			if (this._is_subcontrol) {
				...
				if (this.parent && !this.parent._is_subcontrol && (this.parent._is_containerset || this.parent._is_form)) {
	...
	_pComponent._on_scroll_change = nexacro._emptyFn;
	...
	_pComponent._calcArrangePosition = function () {
		...
		if (info && (obj = info.left)) {
			if (form && form._is_created_contents) {
				...
				this._left = this._convToPixel(obj.distance, comp._getClientWidth());
			}
		}
		else {
			if (this.left != null) {
		...
		if (info && (obj = info.top)) {
			if (form && form._is_created_contents) {
				...
				this._top = this._convToPixel(obj.distance, comp._getClientHeight());
			}
		}
		else {
			if (this.top != null) {
	...
	_pComponent._scrollTo = function (hpos, vpos, bapplyscrollbar, bautohidetimer, evttype, evtkind) {
		...
		if (evtkind != "page_v" && evtkind != "mousewheel_v" && !evttype) {
		...
		if (evtkind != "page_h" && evtkind != "mousewheel_h" && !evttype) {
