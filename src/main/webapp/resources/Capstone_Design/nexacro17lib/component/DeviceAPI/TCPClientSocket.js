...
if (!nexacro.TCPClientSocket) {
	if (nexacro._Browser == "Runtime" && (nexacro._OS == "Windows" || nexacro._OS == "Android" || nexacro._OS == "OSX")) {
		...
		nexacro._readTCPClientSocket = function (obj, isreadline, maxsize, timeout) {
			return nexacro.__readTCPClientSocket(obj, isreadline, maxsize, timeout);
		};
		...
		nexacro._openTCPClientSocket = function (handle, addr, port) {
			...
			params += ',"port":' + port;
			params += '}';
			nexacro.Device.exec('{"id":' + handle + ', "div":"TCPClientSocket", "method":"open", "params":' + params + '}');
			return true;
		};
		nexacro._closeTCPClientSocket = function (handle) {
			nexacro.Device.exec('{"id":' + handle + ', "div":"TCPClientSocket", "method":"close", "params":""}');
			return true;
		};
		nexacro._readTCPClientSocket = function (handle, isreadline, maxsize, timeout) {
			...
			params += '}';
			var result = nexacro.Device.exec('{"id":' + handle + ', "div":"TCPClientSocket", "method":"read", "params":' + params + '}', true);
			return true;
		};
		nexacro._writeTCPClientSocket = function (handle, data, charset) {
			...
			params += '}';
			var result = nexacro.Device.exec('{"id":' + handle + ', "div":"TCPClientSocket", "method":"write", "params":' + params + '}', true);
			return true;
		};
	}
	else {
	...
	_pTCPClientSocket.read = function (size, timeout) {
		if (this._handle && (size != undefined)) {
			var isreadline = false;
			return nexacro._readTCPClientSocket(this._handle, isreadline, size, timeout);
		}
		return Array(false);
	};

	_pTCPClientSocket.readLine = function (maxsize, timeout) {
		if (this._handle && (maxsize != undefined)) {
			var isreadline = true;
			return nexacro._readTCPClientSocket(this._handle, isreadline, maxsize, timeout);
		}
		return Array(false);
	};
	...
	if (nexacro._OS == "iOS" && nexacro._isHybrid()) {
		_pTCPClientSocket.fire_onsuccess = function (params) {
			...
			if (reason == "on_dataarrived") {
				arg0 = nexacro.Device.decodeString(arg0);
			}
