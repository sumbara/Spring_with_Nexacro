...
if (!nexacro._bInitPlatform) {
	...
	__pLayoutManager.checkValid = function (form, szContainer) {
		...
		var correct_type = -1;
		var optimal_layout_num = layout_len - 1;
	...
	__pWindow._on_default_sys_message = function () {
		...
		if (handle) {
			...
			if (nexacro._Browser == "Runtime" && (nexacro._SystemType.toLowerCase() == "win32" || nexacro._SystemType.toLowerCase() == "win64")) {
				...
				if (this.frame && this.frame._is_click_openstatus === false) {
					if (wparam === 0 && this.frame._state_openstatus == 3) {
	...
	__pWindow._updateWrapper = function (brun) {
		nexacro._updateWrapper(this.handle, brun);
	};
		...
		_pApplication._addGlobalVariableFromGlobalValue = function () {
			...
			while (matched = search_regexpr.exec(globalvalues)) {
				...
				switch (variable_type) {
					...
					case "ec":
						var expire;
						nexacro.setCookieVariable(variable_name, variable_value, expire);
						break;
					case "av":
					default:
	...
	_pTouchGestureManager.ontouchstart = function (win, elem, touches, changedTouches, time) {
		...
		if (!touch_input.prevented && this.checkInputPreventDefault(elem, 0)) {
			ret = false;
		}
	...
	_pTouchGestureManager.ontouchmove = function (win, elem, touches, changedTouches, time) {
		...
		if (!touch_input.prevented && this.checkInputPreventDefault(elem, 1)) {
			ret = false;
		}
	...
	_pTouchGestureManager.ontouchend = function (win, elem, touches, changedTouches, time) {
		...
		if (!touch_input.prevented && this.checkInputPreventDefault(elem, 2)) {
			ret = false;
		}
		return ret;
	};
	...
	_pTouchGestureManager.checkInputPreventDefault = function (elem, type) {
		var focused_comp;
	...
	_pEnvironment.setApplicationScale = function (v) {
		var app = nexacro.getApplication();
		if (app && app._isLoaded()) {
			return;
		}

		if (v == 50 || v == 75 || v == 100 || v == 125 || v == 150 || v == 175 || v == 200) {
			nexacro._setApplicationScale(v / 100.0);
		}
	};
	...
	nexacro._setCookieVariable = function (name, value, expire) {
		if (name) {
			nexacro._setLocalStorage(name, value, 4);
		}
	};

	nexacro.removeCookieVariable = function () {
	};
