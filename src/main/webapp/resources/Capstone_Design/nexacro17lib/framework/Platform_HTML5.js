...
if (nexacro._Browser != "Runtime" && !nexacro._init_platform_HTML5) {
	...
	nexacro._syshandler_ontouchmove = function (_sysEvent, node, evt) {
		...
		var touch, touchid, screenX, screenY, curTime, i;
		if (elem) {
			if (!this._allow_default_pinchzoom) {
				evt.preventDefault();
			}
	...
	nexacro.__createOpenWindowHandle = function (_handleParent, name, formurl, left, top, width, height, resizable, layered, taskbar, parentwin, parentframe, frameopener, arr_arg) {
		if (left == null) {
